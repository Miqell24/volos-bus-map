...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: Αστικό ΚΤΕΛ Βόλου Α.Ε. (volos.citybus.gr)' }));
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · Timetables: Αστικό ΚΤΕΛ Βόλου Α.Ε. (volos.citybus.gr)';
      ...
      a.download = `volos-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `volos-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the outer villages would shrink the city to a
  // knot. Open on the city centre with Nea Ionia and the shore in the frame; the
  // Pelion coast to Platanidia is one pan away.
  map.jumpTo({ center: [22.9430, 39.3640], zoom: 12.4 });
}
