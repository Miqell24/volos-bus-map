...
// Volos: ONE feed, and this map builds it itself — see pipeline/citybus-feed.mjs.
// 12 bus lines of Αστικό ΚΤΕΛ Βόλου in the family's navy. No trolleybus, no tram,
// no metro, no rail here.
...
// Exact-match repairs for poles a feed names badly — the Latin-lettered
// multi-word names greekTitleCase leaves alone by design get their case here.
const NAME_FIX = {
  'OLD CITY': 'Old City',
};
...
// ONE feed, ONE cfg.
//
// Volos publishes no GTFS anywhere — the operator's site has timetables and
// nothing else, data.gov.gr carries Athens and Thessaloniki only, and neither
// the MobilityDatabase nor Transitous knows the city. So the feed under
// data/gtfs is written by pipeline/citybus-feed.mjs out of the public backend
// of the operator's own passenger site (volos.citybus.gr → rest.citybus.gr), which
// carries every pole, every line, every pattern and a polyline for each.
//
// allVariants: that API publishes PATTERNS, not runs — 40 patterns across 12 lines,
// each one a branch the operator itself lists. There is no timetable to
// sample, so there is no busiest short-turn to fall for and nothing to throw
// away; drawing them all is the whole network exactly once.
//
// STOP NAMES: the operator shouts them (ΠΛΑΤΕΙΑ ΤΑΧΥΔΡΟΜΕΙΟΥ), and Greek
// capitals drop the accents, so lowercasing alone cannot restore Πλατεία
// Ταχυδρομείου. lib/greek.mjs rewrites them word by word through a dictionary
// of accented forms harvested from the OSM names of the frame (streets,
// villages, churches, schools — data/osm/names.json + the road tiles); what the
// dictionary does not know falls back to plain title case. The poles also
// carry the platform's direction markers — a trailing Μ (μετάβαση, outbound)
// or Ε (επιστροφή, return), and „ΠΡΟΣ ΚΕΝΤΡΟ" (towards the centre) — which
// name the run, not the place, so they come off before the name is printed.

const LBL = new Map();
const isRailTrunk = () => false;

// Words the OSM frame may not spell out for us but every Greek stop list is
// made of — the dictionary learns them here when the extract has no better
// witness (an OSM spelling always wins; this only fills the holes). Folded
// capitals on the left, the accented form on the right.
const SEED = {
  ΠΛΑΤΕΙΑ: 'Πλατεία', ΠΛΑΤΕΙΑΣ: 'Πλατείας', ΣΥΝΟΙΚΙΑ: 'Συνοικία', ΤΕΛΩΝΕΙΟ: 'Τελωνείο',
  ΠΑΛΙΟ: 'Παλιό', ΠΑΛΙΑ: 'Παλιά', ΠΑΛΑΙΟ: 'Παλαιό', ΠΑΛΑΙΑ: 'Παλαιά', ΚΟΙΜΗΤΗΡΙΟ: 'Κοιμητήριο',
  ΝΟΣΟΚΟΜΕΙΟ: 'Νοσοκομείο', ΠΑΝΕΠΙΣΤΗΜΙΑΚΟ: 'Πανεπιστημιακό', ΠΑΝΕΠΙΣΤΗΜΙΟ: 'Πανεπιστήμιο',
  ΣΧΟΛΕΙΟ: 'Σχολείο', ΔΗΜΟΤΙΚΟ: 'Δημοτικό', ΓΥΜΝΑΣΙΟ: 'Γυμνάσιο', ΛΥΚΕΙΟ: 'Λύκειο', ΜΟΥΣΙΚΟ: 'Μουσικό',
  ΣΤΑΣΗ: 'Στάση', ΤΕΡΜΑ: 'Τέρμα', ΑΦΕΤΗΡΙΑ: 'Αφετηρία', ΤΕΛΙΚΗ: 'Τελική', ΚΕΝΤΡΟ: 'Κέντρο',
  ΚΕΝΤΡΙΚΗ: 'Κεντρική', ΚΕΝΤΡΙΚΟ: 'Κεντρικό', ΓΕΦΥΡΑ: 'Γέφυρα', ΕΚΚΛΗΣΙΑ: 'Εκκλησία',
  ΑΓΙΟΣ: 'Άγιος', ΑΓΙΑ: 'Αγία', ΑΓΙΟΥ: 'Αγίου', ΑΓΙΑΣ: 'Αγίας', ΠΑΡΚΟ: 'Πάρκο', ΜΟΥΣΕΙΟ: 'Μουσείο',
  ΣΤΑΘΜΟΣ: 'Σταθμός', ΕΡΓΑΤΙΚΕΣ: 'Εργατικές', ΚΑΤΟΙΚΙΕΣ: 'Κατοικίες', ΔΙΑΣΤΑΥΡΩΣΗ: 'Διασταύρωση',
  ΠΥΛΗ: 'Πύλη', ΟΔΟΣ: 'Οδός', ΟΔΟΥ: 'Οδού', ΛΕΩΦΟΡΟΣ: 'Λεωφόρος', ΝΕΑ: 'Νέα', ΝΕΟ: 'Νέο', ΝΕΟΣ: 'Νέος',
  ΝΕΑΣ: 'Νέας', ΑΝΩ: 'Άνω', ΚΑΤΩ: 'Κάτω', ΠΕΡΙΦΕΡΕΙΑΚΟΣ: 'Περιφερειακός', ΠΑΙΔΙΚΟΣ: 'Παιδικός',
  ΩΔΕΙΟ: 'Ωδείο', ΠΥΡΟΣΒΕΣΤΙΚΗ: 'Πυροσβεστική', ΥΠΗΡΕΣΙΑ: 'Υπηρεσία', ΑΣΤΥΝΟΜΙΑ: 'Αστυνομία',
  ΔΗΜΑΡΧΕΙΟ: 'Δημαρχείο', ΣΤΡΑΤΟΠΕΔΟ: 'Στρατόπεδο', ΑΕΡΟΔΡΟΜΙΟ: 'Αεροδρόμιο', ΛΙΜΑΝΙ: 'Λιμάνι',
  ΑΓΟΡΑ: 'Αγορά', ΛΑΙΚΗ: 'Λαϊκή', ΦΑΡΜΑΚΕΙΟ: 'Φαρμακείο', ΤΡΑΠΕΖΑ: 'Τράπεζα', ΕΘΝΙΚΗ: 'Εθνική',
  ΣΤΡΟΦΗ: 'Στροφή', ΕΙΣΟΔΟΣ: 'Είσοδος', ΕΞΟΔΟΣ: 'Έξοδος', ΓΡΑΜΜΗ: 'Γραμμή', ΣΥΝΕΡΓΕΙΟ: 'Συνεργείο',
  ΞΕΝΟΔΟΧΕΙΟ: 'Ξενοδοχείο', ΚΛΙΝΙΚΗ: 'Κλινική', ΓΗΠΕΔΟ: 'Γήπεδο', ΣΤΑΔΙΟ: 'Στάδιο', ΚΛΕΙΣΤΟ: 'Κλειστό',
  ΠΑΡΚΙΝΓΚ: 'Πάρκινγκ', ΜΕΤΑ: 'Μετά', ΠΡΙΝ: 'Πριν', ΕΠΙ: 'Επί', ΒΙΟΜΗΧΑΝΙΚΗ: 'Βιομηχανική', ΠΕΡΙΟΧΗ: 'Περιοχή',
  ΣΥΝΟΙΚΙΣΜΟΣ: 'Συνοικισμός', ΕΘΝΙΚΗΣ: 'Εθνικής', ΑΝΤΙΣΤΑΣΗΣ: 'Αντίστασης', ΟΚΤΩΒΡΙΟΥ: 'Οκτωβρίου',
  ΟΚΤΩΒΡΗ: 'Οκτώβρη', ΜΑΡΤΙΟΥ: 'Μαρτίου', ΝΟΕΜΒΡΙΟΥ: 'Νοεμβρίου', ΗΡΩΩΝ: 'Ηρώων', ΠΟΛΥΤΕΧΝΕΙΟΥ: 'Πολυτεχνείου',
  ΔΗΜΟΚΡΑΤΙΑΣ: 'Δημοκρατίας', ΕΛΕΥΘΕΡΙΑΣ: 'Ελευθερίας', ΕΙΡΗΝΗΣ: 'Ειρήνης', ΠΑΠΑΣΤΑΥΡΟΥ: 'Παπασταύρου',
  ΕΜΠΟΡΙΚΟ: 'Εμπορικό', ΕΜΠΟΡΙΚΗ: 'Εμπορική', ΓΑΛΑΚΤΟΣ: 'Γάλακτος', ΑΡΧΕΣ: 'Αρχές', ΑΡΧΗ: 'Αρχή', ΓΥΜΝΑΣΙΑ: 'Γυμνάσια',
  ΣΤΡΑΤΙΑΣ: 'Στρατιάς', ΛΑΙΚΕΣ: 'Λαϊκές', ΨΑΡΑΔΙΚΑ: 'Ψαράδικα', ΓΡΑΜΜΕΣ: 'Γραμμές', ΨΥΓΕΙΑ: 'Ψυγεία', ΕΠΙΠΛΑ: 'Έπιπλα',
  ΟΣΠΡΙΑ: 'Όσπρια', ΣΚΕΠΑΣΤΗ: 'Σκεπαστή', ΚΑΛΗ: 'Καλή', ΚΑΡΔΙΑ: 'Καρδιά', ΝΑΠΟΛΕΩΝ: 'Ναπολέων', ΦΟΡΤΟΥΝΑ: 'Φορτούνα',
  ΛΙΒΑΔΑΚΙ: 'Λιβαδάκι', ΦΑΡΟΣ: 'Φάρος', ΠΟΛΥΚΑΤΟΙΚΙΕΣ: 'Πολυκατοικίες', ΤΑΧΥΔΡΟΜΕΙΟΥ: 'Ταχυδρομείου', ΤΑΧΥΔΡΟΜΕΙΟ: 'Ταχυδρομείο',
  ΕΚΚΟΚΙΣΤΗΡΙΟ: 'Εκκοκκιστήριο', ΝΟΣΤΟΣ: 'Νόστος', ΧΕΛΙΔΩΝΗ: 'Χελιδώνη', ΣΥΓΚΟΙΝΩΝΙΩΝ: 'Συγκοινωνιών', ΣΥΓΚΟΙΝΩΝΙΕΣ: 'Συγκοινωνίες',
  ΥΔΡΑΣ: 'Ύδρας', ΛΕΧΩΝΙΑ: 'Λεχώνια', ΠΛΑΖ: 'Πλαζ', ΜΕΡΙΑ: 'Μεριά', ΑΛΛΗ: 'Άλλη', ΜΕΛΙΣΣΑΤΙΚΑ: 'Μελισσάτικα', ΑΛΙΒΕΡΙ: 'Αλιβέρι',
  ΑΛΥΚΩΝ: 'Αλυκών', ΑΛΥΚΕΣ: 'Αλυκές', ΥΨΗΛΑΝΤΟΥ: 'Υψηλάντου', ΥΨΙΛΑΝΤΟΥ: 'Υψηλάντου', ΣΤΑΘΜΟΥ: 'Σταθμού', ΓΕΩΡΓΙΚΗ: 'Γεωργική',
  ΣΧΟΛΗ: 'Σχολή', ΕΠΙΣΤΗΜΩΝ: 'Επιστημών', ΥΓΕΙΑΣ: 'Υγείας', ΠΑΝΕΠΙΣΤΗΜΙΑΚΟΥ: 'Πανεπιστημιακού', ΝΟΣΟΚΟΜΕΙΟΥ: 'Νοσοκομείου',
  ΚΟΙΜΗΤΗΡΙΟΥ: 'Κοιμητηρίου', ΠΑΡΚΟΥ: 'Πάρκου', ΓΗΠΕΔΟΥ: 'Γηπέδου', ΣΤΑΔΙΟΥ: 'Σταδίου', ΔΗΜΑΡΧΕΙΟΥ: 'Δημαρχείου',
};
// The accent dictionary — see lib/greek.mjs. Built once, from every named OSM
// node of the frame and every named road of the four tiles.
const nameDict = (() => {
  const docs = [];
  for (const f of ['data/osm/names.json', ...Array.from({ length: 4 }, (_, i) => `data/osm/tiles/t${i + 1}.json`)]) {
    const p = join(ROOT, f);
    if (!existsSync(p)) continue;
    try { docs.push(JSON.parse(readFileSync(p, 'utf8'))); } catch { /* unreadable extract: skip */ }
  }
  const d = buildNameDict(docs);
  const fromOsm = d.size;
  const fold = (w) => w.normalize('NFD').replace(/[̀-ͯ]/g, '').toUpperCase();
  const accented = (w) => /[άέήίόύώΆΈΉΊΌΎΏϊϋΐΰ]/.test(w);
  // the seed fills holes, and also beats an OSM spelling that carries no accent
  // at all — one sloppy tag must not teach the whole map to write Πυλη
  for (const [k, v] of Object.entries(SEED)) if (!d.has(fold(k)) || !accented(d.get(fold(k)))) d.set(fold(k), v);
  log(`Greek names: dictionary of ${d.size} accented word forms (${fromOsm} from ${docs.length} OSM extract(s), ${d.size - fromOsm} seeded)`);
  return d;
})();
let dirMarkersStripped = 0, accentRepairs = 0;
const greekName = (n) => {
  let s = String(n || '').replace(/\s+/g, ' ').trim()
    // the platform's typing habits: an ordinal glued to the next word
    // (4οΓΥΜΝΑΣΙΟ), a parenthesis glued to the word before it
    .replace(/(\d+[οηα])(?=[Α-ΩΆΈΉΊΌΎΏ])/g, '$1 ').replace(/(\S)\(/g, '$1 (');
  // direction markers, never the whole name: a "… ΠΡΟΣ ΚΕΝΤΡΟ" / "… ΠΡΟΣ
  // ΝΟΣΟΚΟΜΕΙΟ" tail (towards …), a parenthesis that says which way the bus
  // is heading — "Δημαρχείο(επί της 2ας Νοεμβρίου προς Ν.Ι.)" — and the
  // platform's trailing Μ (μετάβαση, outbound) or Ε (επιστροφή, return)
  // (no \b here: JavaScript's word boundary is ASCII-only and never fires on Greek)
  const t = s.replace(/\s*\([^)]*προς[^)]*\)/i, '').replace(/\s+ΠΡΟΣ\s+\S.*$/i, '').replace(/\s+[ΜΕME]$/, '').trim();  // Greek Μ/Ε or their Latin look-alikes
  if (t !== s && t.length >= 3) { s = t; dirMarkersStripped++; }
  const out = NAME_FIX[s] || greekTitleCase(s, nameDict);
  // The source also writes some names in lowercase but without the accent
  // (Τερμα, Αγιος, Μερια). Every Greek word of two syllables or more carries
  // exactly one, so an accentless word the dictionary knows is a typo, and
  // the dictionary's spelling — the same letters, plus the accent — replaces it.
  // (the class must include the accented letters — ά…ώ sit outside α-ω in
  // Unicode, and a class without them cuts Αφετηρία into Αφετηρ + ία)
  return out.replace(/[Α-Ωα-ωΆΈΉΊΌΎΏάέήίόύώϊϋΐΰς]{4,}/g, (w) => {
    if (/[άέήίόύώΆΈΉΊΌΎΏϊϋΐΰ]/.test(w) || w === w.toUpperCase()) return w;
    const k = w.normalize('NFD').replace(/[̀-ͯ]/g, '').toUpperCase();
    const known = nameDict.get(k);
    if (!known || !/[άέήίόύώΆΈΉΊΌΎΏϊϋΐΰ]/.test(known) || known.normalize('NFD').replace(/[̀-ͯ]/g, '').toUpperCase() !== k) return w;
    const fixed = w[0] === w[0].toUpperCase() ? known[0].toUpperCase() + known.slice(1) : known.toLowerCase();
    if (fixed !== w) accentRepairs++;
    return fixed;
  });
};
...
const MODES = [{
  ...
  // Nea Ionia and Dimini west, Ano Volos, Alli Meria and Melissatika north,
  // the Pelion coast east as far as Lechonia and Platanidia — 2 × 2 tiles
  ...
  feeds: [
    { tag: 'vl', dir: 'data/gtfs', routeTypes: ['3'], allVariants: true,
      // the key is the number the flag shows, bare (citybus-feed.mjs has
      // already dropped the system's leading zero: 01 → 1)
      mapKey: (sn) => (sn || '').replace(/\s+/g, ' ').trim() || null,
      nameFix: greekName },
  ],
}];
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    //
    // A second trigger, learnt in Burgas, whose planner ships an OVERVIEW polyline —
    // simplified, the way a routing API returns one — and simplification eats
    // curvature. Under a kilometre that is harmless: the engine bridges the
    // hole by routing and the line lands on the road within two metres. Over
    // SHAPE_GAP_MAX the polyline has stopped describing a road at all and is
    // just a chord across open country (line 3 to Banevo: one 3.7 km straight
    // over the fields, which the router could not bridge and which was drawn
    // raw). The stop chain from the same API is the better witness there —
    // 24 poles over 19 km, one every 800 m, each on its real coordinate — so
    // that pattern matches like a shapeless feed and the routing between the
    // poles draws the road.
    const SHAPE_GAP_MAX = 2000;
    const worstGap = (pts) => {
      let w = 0;
      for (let i = 1; i < pts.length; i++) {
        const d = Math.hypot((pts[i][0] - pts[i - 1][0]) * 111320,
    ...
    for (const r of feedReps) {
      const gap = r.shapeLatLon && r.shapeLatLon.length >= 2 ? worstGap(r.shapeLatLon) : 0;
      if (gap > SHAPE_GAP_MAX) {
        log(`${r.line}/${r.dir}: overview polyline jumps ${Math.round(gap)} m — matching on the stop sequence instead`);
      } else if (r.shapeLatLon && r.shapeLatLon.length >= 2) continue;
      r.pseudo = true;
